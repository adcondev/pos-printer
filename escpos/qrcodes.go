...
func (c *Protocol) SelectQRModel(model QRModel) ([]byte, error) {
	...
	cmd := make([]byte, 0, 9)
	cmd = append(cmd, shared.GS, '(', 'k') // Comando QR
...
func (c *Protocol) SetQRData(data string) ([]byte, error) {
	...
	pL, pH := shared.ToLittleEndian(uint16(len(data) + 3)) // nolint:gosec
